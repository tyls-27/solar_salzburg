...
ltw <- ltw_clean %>%
  ...
  mutate(total_stimmen = rowSums(across(ÖVP:MFG))) %>% 
...
gemeinden_data <- gemeinden_voll %>%
  ...
  mutate(einwohnerzahl_pro_qkm = bev?lkerung/as.numeric(set_units(flaeche, "km^2")), .after = bevölkerung) %>%
